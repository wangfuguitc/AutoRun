...
def run_case(case_list, report_dir):
    ...
    for case in case_list:
        ...
        with open(report_html, 'a') as handle:
            if sub.returncode:
                html_report(case, 'fail', handle, time)
            else:
                html_report(case, 'success', handle, time)
    with open(report_html, 'a') as handle:
        handle.write('''</table></body></html>''')


def html_report(case, state, handle, time):
    ...
    content = content.replace('file_path', case).replace('log_path', log).replace('state', state).replace('time', str(time))
